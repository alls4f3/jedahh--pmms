...
namespace AmtarApp.Modules.Common
{
    public class autoApprove48
    {
        ...
        public string CUserId() {
            var stgVar = ConfigurationManager.AppSettings.Get("autoApprove48User");
        ...
        public void Move()
        {
            ...
            var stgVar = ConfigurationManager.AppSettings.Get("autoApprove48");

            foreach (string item in stgVar.Split('#'))
            {
               
                string subItem = item.Split('*')[0];
                string subItem2 = item.Split('*')[1];
...
select WorkOrderId ,
			...
			'Auto Approve from System after 48H' as Remark3,Depid as Department

			from workorder
			where 
workorder.depid='"+ subItem + @"'
			and
			(workorder.WorkflowStage in ( " + subItem2 + @" ) and DATEDIFF(HOUR, LastUpdate,GETDATE()) >=48 )
