...
{
    public class autoUpdateDirctor
    {
...
select WorkOrderId ,
			...
			'Auto Update from the System' as Remark3,Depid as Department

			from workorder
			where 

			--(workorder.WorkflowStage  in ( select StageKey from WorkFlow_Stages where usertype='5' ) and RefId in (select Projects.ProjectId from Projects where Projects.DEPT_NO='6581'))

			--or 
			(workorder.WorkflowStage in ( select StageKey from WorkFlow_Stages where usertype='5' ) and DATEDIFF(HOUR, LastUpdate,GETDATE()) >=48 and RefId	in (select Projects.ProjectId from Projects where Projects.DEPT_NO='1133'))
