...
namespace PMMS.Administration.Endpoints
{
    ...
    public class UserController : ServiceEndpoint
    {
        ...
        {
            if (ConfigurationManager.AppSettings.Get("EnableSSOPassword") == "1")
            {

                request.Entity.Password = request.Entity.Username + "pass@104[p*01";
                request.Entity.PasswordConfirm = request.Entity.Username + "pass@104[p*01";
            }
        ...
        public SaveResponse Update(IUnitOfWork uow, SaveRequest<MyRow> request)
        {
            ...
            if (request.EntityId.ToString() == "97")
            {
                throw new ValidationError("you cannot edit this user  .it's used by the system");
            }

            if (request.EntityId.ToString() == "98")
            {
                throw new ValidationError("you cannot edit this user  .it's used by the system");
            }
           

            if (request.EntityId.ToString() == "106")
            {
                throw new ValidationError("you cannot edit this user .it's used by the system");
            }
        ...
        public DeleteResponse Delete(IUnitOfWork uow, DeleteRequest request)
        {
            if(request.EntityId.ToString() == "97")
            {
                throw new ValidationError("you cannot delete this user  .it's used by the system");
            }
            ...
            if (request.EntityId.ToString() == "1")
            {
                throw new ValidationError("you cannot delete this user .it's used by the system");
            }
